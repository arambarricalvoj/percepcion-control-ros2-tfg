...
class GidatzeProbaWrapper(Node):
    ...
    def posizioa_kalkulatu(self):
        ...
        self.posicion_rover = (posicion_x, posicion_y)
    ...
    def ruta_optima(self):#, punto_inicial, matriz_puntos):
        matriz_actualizada = np.vstack((self.posicion_rover, self.waypoints))
        permutaciones = itertools.permutations(matriz_actualizada)

        mejor_ruta = None
        menor_distancia = float('inf')

        for permutacion in permutaciones:
            matrix = np.array(permutacion)
            if not np.array_equal(matrix[0, :], self.posicion_rover):
                continue
            dist_actual_matrix = cdist(matrix, matrix, metric='euclidean')
            dist_actual = np.sum(np.diag(dist_actual_matrix, k=-1))
    ...
    def arucoBilatu(self, goal_handle):
        ...
        while angulo_total < (90 * bira_kopurua)-desfase:
            rclpy.spin_once(self, timeout_sec=0.1) #?
            ...
            if self.imu_datu_berriak:
                ...
                if oraingo_angelua != 0.0 and oraingo_angelua > 0:
                    if noranzkoa:
                        diferencia = (oraingo_angelua - anterior + 90) % 90
                    else:
                        diferencia = (anterior - oraingo_angelua + 90) % 90
        ...
        ruta, _ = self.ruta_optima()
        target_point = ruta[0]
        
        x1 = self.posicion_rover[0]
        y1 = self.posicion_rover[1]
        x2 = target_point[0]
        y2 = target_point[1]
        ...
        distancia = dx / math.cos(target_angle+self.hasierakoAngelua)
        tiempo =  7#distancia / (3/7)
        self.get_logger().info(f"TIEMPO {tiempo}")

        self.get_logger().info(f"TARGET ANGLE {target_angle}")
        current_angle = self.imu_datuak.euler_angeluak.x

        direction = 'left' if ((target_angle - current_angle + 360) % 360) > 180 else 'right'
        ...
        while abs((target_angle+self.hasierakoAngelua - current_angle + 180) % 360 - 180) > 0.3:
            rclpy.spin_once(self, timeout_sec=0.1) #?
            ...
            if self.imu_datu_berriak:
                ...
                self.get_logger().info(f"Rotando desde {current_angle:.2f}º hasta {target_angle:.2f}º")
                
                current_angle = self.imu_datuak.euler_angeluak.x
                self.get_logger().info(f"Rotando desde {current_angle:.2f}º hasta {target_angle:.2f}º")
                self.get_logger().info(f"Orientación ajustada a {current_angle:.2f}º")
        
        self.motorrak_gelditu()
        msg = ServoakMugitu()
        msg.angeluak = self.servo_posizioak_kalkulatu(0)
        self.servo_publisher.publish(msg)
        self.get_logger().info(f'Moviendo servos a: {msg.angeluak}')
        self.motorrak_mugitu([-30, 30, -30, 30, -30, 30])
        denbora = time.time()
        hasierako_denbora = denbora
        while denbora - hasierako_denbora < tiempo:
            #rclpy.spin_once(self, timeout_sec=0.1) #?
            denbora = time.time()
            continue
        self.motorrak_gelditu()
        print(f"Navegando desde ({x1}, {y1}) a ({x2}, {y2})")
